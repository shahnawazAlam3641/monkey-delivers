  ...
  return (
    <div className=' max-w-[80%] mx-auto flex flex-col gap-8 p-4'>
        <div className=' mx-auto mt-8 w-[100%] flex justify-center gap-2'>
          <input type='text' value={searchText} onChange={handleSearch} className="rounded-lg w-[100%] max-w-[850px] border-2 p-3  " placeholder='Search for Restaurants and Food'/>
        ...
        {sugg && (
      <div className={`flex flex-col gap-4 items-center ${searchText ? 'flex' : 'hidden'}`}>
        {sugg.data.suggestions.map((suggestion, index) => (
          
          <div key={index} className='w-[80%] flex gap-4 p-2  hover:bg-slate-200 rounded-md'>
        ...
        <div className='flex  overflow-scroll overflow-y-hidden'>
            {info.map((searchCuisine)=>{
                return <img key={searchCuisine.id} className='w-24' src={"https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_288,h_360/" + searchCuisine.imageId}/>
            })}
        </div>
        </div>
    </div>
